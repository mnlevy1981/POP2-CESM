...
!-----------------------------------------------------------------------
    ...
    integer (int_kind) ::     &
        tavg_UVEL_INTERIOR,   & ! tavg_id for UVEL restoring value term
        tavg_VVEL_INTERIOR,   & ! tavg_id for VVEL restoring value term
        tavg_UVEL_INTERIOR_2, & ! tavg_id for UVEL restoring value term
        tavg_VVEL_INTERIOR_2    ! tavg_id for VVEL restoring value term

    real (r8), dimension(:,:,:,:,:), allocatable :: &
        UVEL_INTERIOR_DATA, & ! data to use for interior restoring UVEL
        VVEL_INTERIOR_DATA    ! data to use fof interior restoring VVEL 
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
       ...
       write(stdout,*) ' forcing_uv_interior_nml namelist settings:'
       write(stdout,blank_fmt)
       write(stdout, forcing_uv_interior_nml)
       write(stdout,blank_fmt)
   endif
